...
namespace InputManagement
{
    ...
    public class ClickManager
    {
        ...
        static KeyBindKey[] keys = new KeyBindKey[0];
        static float keyPressTimeout = 0.18f; //The time (in seconds) to press the same key again to make it count as double click.
        public static bool DoubleClick(KeyBindKey key)
        {
            ...
            for (int forg = 0; forg < keys.Length; forg++)
            {
                ...
                if (keyEPress[forg] < Time.time) { keys[forg] = KeyBindKey.None; keys[forg] = 0; } else { availableKeys++; }
            }
            //Funnels array values into free spaces - Removes "KeyCode.None" elements
            KeyBindKey[] nKeys = new KeyBindKey[availableKeys]; float[] nKeyEPress = new float[availableKeys];
            for (int funn = 0; funn < keys.Length; funn++)
            {
                //If the current key is not "null"
                if (keys[funn] != KeyBindKey.None)
                {
                    //Looks at all nKey elements
                    for (int fum = 0; fum < availableKeys; fum++)
                    {
                        //If the element is "null"
                        if (nKeys[fum] == KeyBindKey.None)
                        {
            ...
            if (!revisedKey)
            {
                ...
                KeyBindKey[] newKeys = new KeyBindKey[keys.Length + 1];
