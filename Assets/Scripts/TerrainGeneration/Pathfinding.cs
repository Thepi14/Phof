...
namespace Pathfindingsystem
{
    ...
    public class Pathfinding
    {
        ...
        public List<PathNode> FindPathB(int startx, int starty, int endx, int endy, Func<int, int, bool> booleaner = null)
        {
            ...
            startNode.gCost = 0;
            startNode.hCost = CalculateDistanceCostB(startNode, endNode);
            startNode.CalculateFCost();
            ...
            while (openList.Count > 0)
            {
                ...
                foreach (PathNode neighbourNode in GetNeighbourList(currentNode))
                {
                    ...
                    if (currentNode != null && neighbourNode != null) { }
                        ...
                        continue;
                    int tentativeGCost = currentNode.gCost + CalculateDistanceCostB(currentNode, neighbourNode);
                    if (tentativeGCost < neighbourNode.gCost)
                    {
                        neighbourNode.cameFromNode = currentNode;
                        neighbourNode.gCost = tentativeGCost;
                        neighbourNode.hCost = CalculateDistanceCostB(neighbourNode, endNode);
                        neighbourNode.CalculateFCost();
        ...
        private int CalculateDistanceCostB(PathNode a, PathNode b)
        {
            if (a != null && b != null)
            {
                int xDistance = Mathf.Abs(a.x - b.x);
                int yDistance = Mathf.Abs(a.y - b.y);
                int remaining = Mathf.Abs(xDistance - yDistance);
                return (MOVE_DIAGONAL_COST * 10) * Mathf.Min(xDistance, yDistance) + MOVE_STRAIGHT_COST + remaining;
            }
            return 0;
        }
